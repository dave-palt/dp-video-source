...
// copied from https://github.com/coska/rust-youtube-dl/blob/master/src/lib.rs
// he did not publish it on cargo so I copied it here
//
...
pub fn get_ytplayer_config(webpage_contents: String) -> Result<String, &'static str> {
    let patterns = [r"var ytInitialPlayerResponse\s*=\s*(\{.+?\});"];
...
pub async fn extract_video_data(player_config: JsonValue) -> Option<VideoInfo> {
    ...
    for streaming in streaming_datas.members() {
        extract_streaming_data(streaming, &video_info).await;
    }
...
async fn extract_streaming_data(streaming_format: &JsonValue, video_info: &VideoInfo) {
    video_info.append_streaming_data(StreamingData {
        ...
        statuscode: get_status_code(&streaming_format["url"].as_str().unwrap_or(""))
            .await
            .unwrap_or("N/A".to_string()),
    });
}

pub async fn get_status_code(url: &str) -> Result<String, String> {
